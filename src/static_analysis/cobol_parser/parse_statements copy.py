...
def parse_statement(ctx):
    ...
    statement_map = {
        ...
        Cobol85Parser.SortStatementContext: visit_sort_statement_context,
    }
...
def visit_sort_statement_context(ctx):
    keys = []
    onKeyClause = ctx.sortOnKayClause()
    for child in context_info.get_children(onKeyClause):
        if isinstance(child, Cobol85Parser.SortKeyContext):
            keys.append(visit_sort_key_context(child))
    return keys

def visit_sort_key_context(ctx):
    """ Επισκέπτεται ένα SortKeyContext και επιστρέφει το αντίστοιχο statement object. """
    # logger.info("-------visitSortKeyContext-----------")
    sort_key = Statement()
    for child in context_info.get_children(ctx):
        if isinstance(child, Cobol85Parser.IdentifierContext):
            sort_key = visit_identifier_context(child)
        elif isinstance(child, Cobol85Parser.SortOrderContext):
            sort_key = child.getText()
    return sort_key
